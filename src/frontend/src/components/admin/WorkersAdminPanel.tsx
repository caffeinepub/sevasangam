...
import type { Inquiry, WorkerProfile } from "../../backend";
import { InquiryStatus } from "../../backend";
import {
  useApproveWorker,
  useGetAllCategories,
  useGetAllInquiriesAdmin,
  useGetAllWorkersAdmin,
  usePublishWorker,
  useRejectWorker,
  useRemoveWorker,
  useUnpublishWorker,
  useUpdateInquiry,
  useUpdateWorkerCategoryAdmin,
...
export default function WorkersAdminPanel() {
  ...
  const [completingInquiryId, setCompletingInquiryId] = useState<string | null>(
    null,
  );
  ...
  if (workers.length === 0) {
    return (
      <EmptyState
        ...
        description="Workers will appear here once they register."
      />
    );
  }
  ...
  return (
    <div className="space-y-6">
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                      {getCategoryName(worker.category_id)} •{" "}
              ...
              <CardContent className="space-y-4">
                <div className="flex flex-wrap gap-2">
                  ...
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => handleOpenCategoryDialog(worker)}
                  >
                ...
                {workerInquiries.length > 0 && (
                  <div className="border-t pt-4 mt-4">
                    <h4 className="text-sm font-semibold mb-3">
                      Assigned Jobs ({workerInquiries.length})
                    </h4>
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <span className="text-xs text-muted-foreground">
                                {new Date(
                                  Number(inquiry.created_at) / 1000000,
                                ).toLocaleDateString()}
                              </span>
                            </div>
                            ...
                            <p className="text-xs text-muted-foreground">
                              {inquiry.customer_contact || "No contact"}
                            </p>
      ...
      <Dialog
        ...
        onOpenChange={(open) => setChangeCategoryDialog({ open, worker: null })}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Change Worker Category</DialogTitle>
            <DialogDescription>
              Update the service category for{" "}
              {changeCategoryDialog.worker?.full_name}
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
            <Select
